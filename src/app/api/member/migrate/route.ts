...
export async function POST(req: NextRequest) {
  ...
  if (error) {
    console.error('[api/member/migrate] Erreur requête:', error);
  ...
  // Envoi en batch de 5 emails en parallèle
  const BATCH_SIZE = 5;
  for (let i = 0; i < legacyMembers.length; i += BATCH_SIZE) {
    const batch = legacyMembers.slice(i, i + BATCH_SIZE);
