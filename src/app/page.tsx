...
function LandingContent() {
  ...
  const handleClearScanner = () => {
    if (!confirm('Réinitialiser votre session ?')) return;
  ...
  const isReturningScanner = scannerName && hasMemberId;
  ...
  return (
    <>
      <div className="z-10 flex flex-col items-center text-center gap-10 w-full max-w-xs">
        ...
        {/* CTA principal */}
        <div className={`w-full flex flex-col gap-3 transition-all duration-500 ${ready ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
          {isReturningScanner ? (
            <Link href="/unlock"
